...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onApplySetCount, onStartTimed }) {
  ...
  const cell = (s, i, col, cls) => (
    <div className={'stp ' + cls}>
      <button type="button" aria-label={t('Decrease {0}', col.hd)}
        onClick={() => bump(s, i, col, -1)}><Icon name="minus" /></button>
      ...
      <span className="val"><NumberField decimal={col.dec} nullable={col.opt} value={s[col.f] ?? ''}
        aria-label={col.hd} onChange={v => onField(i, col.f, col.eff ? capEffort(col.eff, v) : v)} /></span>
      <button type="button" aria-label={t('Increase {0}', col.hd)}
        onClick={() => bump(s, i, col, 1)}><Icon name="plus" /></button>
    </div>
  )
  return <>
...
function ActiveWorkout() {
  ...
  return <div className="narrow">
    <div className="hdr">
      <button className="iconbtn" aria-label={t('Discard')} onClick={() => confirmSheet({ title: t('Discard workout?'), message: t('The sets you logged in this session will be lost.'), confirmText: t('Discard'), danger: true, onConfirm: discardActiveWorkout })}><Icon name="xmark" /></button>
