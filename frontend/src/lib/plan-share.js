...
import { portableEquipmentUse } from './equipment-load.js'
...
function cleanEx(e, progressionGroup) {
  ...
  // A friend's profile has different ids and inventory. Preserve only the portable meaning
  // of the logged number and the catalog category that may be matched locally.
  const equipmentUse = portableEquipmentUse(e.equipmentUse)
  if (equipmentUse) o.equipmentUse = equipmentUse
  if (e.sg) o.sg = e.sg
  // This opaque plan-local token preserves only the user's sharing partition. Runtime ids,
  // workouts, controls and weights remain private and are regenerated by the recipient.
  if (progressionGroup) o.progressionGroup = progressionGroup
  return o
}
...
export function buildPlanBundle(S, name) {
  ...
  let nextGroup = 1
  const tokenFor = (exercise, routine) => {
    const explicit = typeof exercise.progressionId === 'string' && exercise.progressionId
      ? `explicit:${exercise.progressionId}`
      : null
    let key = explicit
    if (!key) {
      const signature = progressionConfigSignature(exercise, routine, S)
      const compatible = inferredGroups.find(group =>
        group.signature === signature && !group.routines.has(routine)
      )
      if (compatible) {
        compatible.routines.add(routine)
        key = compatible.key
      } else {
        key = `inferred:${routine.id || inferredGroups.length}:${inferredGroups.length}`
        inferredGroups.push({ key, signature, routines: new Set([routine]) })
      }
    }
    if (!groupTokens.has(key)) groupTokens.set(key, `g${nextGroup++}`)
    return groupTokens.get(key)
  }
  const routines = (S.routines || []).map(r => ({
    ...
    emoji: r.emoji,
    ...(r.prog ? { prog: r.prog } : {}),
    ex: (r.ex || []).map(e => cleanEx(e, tokenFor(e, r)))
  }))
...
export function parsePlan(raw) {
  ...
  const publicExerciseConfig = e => {
    ...
    const {
      ...
      equipmentUse: rawEquipmentUse,
      equipmentUses: _equipmentUses,
      ...config
    } = e
    const progressionGroup = typeof rawProgressionGroup === 'string'
      && /^[a-zA-Z0-9_-]{1,80}$/.test(rawProgressionGroup)
      ? rawProgressionGroup
      : null
    const equipmentUse = portableEquipmentUse(rawEquipmentUse)
    return {
      ...config,
...
export function mergePlan(s, bundle, { schedule } = {}) {
  ...
  bundle.routines.forEach(r => {
    ...
    imported.push({
      ...
      ex: (r.ex || []).map(e => ({
        ...
        routineExerciseId: ROUTINE_EXERCISE_PREFIX + uid()
      }))
    })
  })

  // Every imported partition receives a fresh local group, isolated from existing routines.
  // Old plan files without tokens infer sharing from equivalent configs, still never from local
  // runtime ids. Same-routine duplicates remain separate.
  const declaredGroups = new Map()
  const inferredGroups = []
  const freshImportedGroup = () => PROGRESSION_PREFIX + 'import:' + uid()
  imported.forEach(routine => {
    routine.ex.forEach(config => {
      const declared = config.progressionGroup
      const signature = progressionConfigSignature(config, routine, s)
      let progressionId
      if (declared) {
        if (!declaredGroups.has(declared)) declaredGroups.set(declared, freshImportedGroup())
        progressionId = declaredGroups.get(declared)
      } else {
        const compatible = inferredGroups.find(group =>
          group.signature === signature && !group.routines.has(routine)
        )
        if (compatible) {
          progressionId = compatible.progressionId
          compatible.routines.add(routine)
        } else {
          progressionId = freshImportedGroup()
          inferredGroups.push({ progressionId, signature, routines: new Set([routine]) })
        }
      }
      delete config.progressionGroup
      config.progressionId = progressionId
      config.progressionSignature = signature
    })
  })
  s.routines.push(...imported)
  if (schedule) {
  ...
  // Reconcile malformed/hostile group tokens and seed any safe operational defaults.
